...
inline unsigned short do_phase_1_move(unsigned long long moveses,
	...
	unsigned int twist = 0, flip = 0, ud_slice = 0, ud = 0;
	unsigned int index = 0;
	unsigned int flipslice;

	unsigned short flipslice_class = 0;
	unsigned short flipslice_symmetry = 0;
	...
	for (short x = 0; x != i; x++) {
		...
		ud_slice = ud_slice_phase_2_table[18 * ud_slice + m];

		ud = ud_slice / 24;

		flipslice = (ud << 11) + flip;
	...
	if (flipslice_twist_depth3[index] == 30 && pass < 9) {
	...
	else if (flipslice_twist_depth3[index] != 30 && pass > 8) {											//this is for backwards
		flipslice_twist_depth3[flipslice_class_index] = flipslice_twist_depth3[index] + i;


		sym = fs_sym[flipslice_class_index];
		...
		if (sym != 1) {
			...
			for (unsigned short j = 0; j != 16; j++) {
				...
				if (sym % 2 == 1) {
					...
					if (flipslice_twist_depth3[index_new] == 30) {
						flipslice_twist_depth3[index_new] = flipslice_twist_depth3[flipslice_class_index];
					}
				}
			}
		}
	...
	if (flipslice_twist_depth3[index] <= i) {
		return 4000000000;											//This checks that the new mapping is one more than the last
	}																//otherwise itll mean that it's already filled in
	

	//it has to be 30 otherwise it would have broken from this function already									
...
void make_phase_1_pruning_table(){
	...
	bool something = true;
		...
		for (unsigned int i = 0; i != 64430; i++) {
			...
			for (unsigned char x = 0; x != 16; x++) {
				sy.edge_perm = symmetries[x].edge_perm;
				sy.edge_ori = symmetries[x].edge_ori;
	...
	std::cout << "enter the machine to do start from 0: ";
	std::cin >> machine;

	unsigned short total_machine;
	std::cout << "enter the total number of machines: ";
	std::cin >> total_machine;
	...
	while (pass != 13) {
		if (pass < 9) {
			...
			for (unsigned short i = 18; i != pass + 18; i++) {
				total_moves = i * total_moves + 18;
			}

			for (unsigned long long moveses = (total_moves * machine)/total_machine; moveses != (total_moves * (machine + 1)) / total_machine; moveses++) {
				index = do_phase_1_move(moveses, twist_table, flip_table, ud_slice_phase_2_table,
					flipslice_sym_classes, flipslice_symmetry, sym_twist_conversion, flipslice_sym_rep, pass, 0);
			...
			for (unsigned short i = 18; i != 3 + 18; i++) {
				total_moves = i * total_moves + 18;
			}
			...
			for (unsigned int i = 0; i != sizeof(flipslice_twist_depth3); i++) {
				if (flipslice_twist_depth3[i] == 30) {
					for (unsigned long long moveses = 0; moveses != total_moves; moveses++) {
						if (do_phase_1_move(moveses, twist_table, flip_table, ud_slice_phase_2_table,
							flipslice_sym_classes, flipslice_symmetry, sym_twist_conversion, flipslice_sym_rep, pass, i) == 0) { //the scenario it retraces to a cube of known distance
							break;
						}
					}
				}
